 ...
 *
 * Description:
 *   AXI4-Lite Slave MMIO register block for host CPU orchestration of the minimal NPU.
 *   Provides clean handshaking, configuration registers, self-clearing execution triggers,
 *   and status/interrupt reporting.
...
module npu_min_csr #(
    ...
    // AXI-Lite Write Channels (Synchronous Reset)
    ...
    // AXI-Lite Read Channels (Synchronous Reset)
    always_ff @(posedge clk_i) begin
        if (!rst_n) begin
            s_axil_arready <= 1'b1;
            s_axil_rvalid  <= 1'b0;
