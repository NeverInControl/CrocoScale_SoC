...
module npu_soft_regs #(
    ...
    always_ff @(posedge clk_i or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // Register write commit when both address and data are received
            if ((aw_done || (s_axil_awvalid && ~aw_done)) && 
                (w_done  || (s_axil_wvalid && ~w_done)) && !s_axil_bvalid) begin
                
                logic [7:0] target_addr;
                target_addr = aw_done ? latched_waddr : s_axil_awaddr[7:0];

                case (target_addr)
                    REG_OFFSET_CTRL: begin
                        reg_ctrl <= s_axil_wdata;
                        if (s_axil_wdata[0]) start_pulse_o <= 1'b1;
                    end
                    ...
                    REG_OFFSET_QUANT_PARAM: reg_quant_param <= s_axil_wdata;
                    REG_OFFSET_CONFIG:      reg_config      <= s_axil_wdata;
                    REG_OFFSET_IRQ_STATUS:  if (s_axil_wdata[0]) reg_irq_status <= 1'b0;
                    default: ;
                endcase
