...
module tb_npu_im2col;
    ...
    initial begin
        ...
        resolve_mem_dir("im2col_conv3x3_act.mem", mem_dir);
        $display("[INFO] Resolved memory directory: '%s'", mem_dir);

        // Verify test vectors exist
        check_file_exists({mem_dir, "im2col_conv3x3_act.mem"});
        check_file_exists({mem_dir, "im2col_conv3x3_w.mem"});
        check_file_exists({mem_dir, "im2col_conv3x3_b.mem"});
        check_file_exists({mem_dir, "im2col_conv3x3_cfg.mem"});
        check_file_exists({mem_dir, "im2col_conv3x3_out_quant.mem"});
