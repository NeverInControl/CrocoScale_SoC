module tb_npu_full_sequencer;
	localparam signed [31:0] ARRAY_HEIGHT = 8;
	localparam signed [31:0] ARRAY_WIDTH = 8;
	...
	wire [31:0] crossbar_sel;
	wire [71:0] act_sram_addr;
	wire [7:0] act_sram_we;
	...
	reg signed [31:0] err_count;
	reg signed [31:0] sim_cycle;
	reg signed [31:0] pass_seen;
	...
	initial begin
		...
		while (busy) begin
			...
			if (dut.fsm_inst.state_o == 3'd2) begin
				if (current_pass != pass_seen)
					pass_seen = current_pass;
				if (cycle_in_pass < 256) begin
					if (!array_en || !psum_systolic_en) begin
						$display("[FAIL 3x3] Cycle %0d: Expected array_en and psum_systolic_en at cycle_in_pass %0d", sim_cycle, cycle_in_pass);
						err_count = err_count + 1;
					end
				end
		...
		if (err_count == 0)
			$display(">>> SUCCESS: 3x3 Mode Verified 100%%! <<<");
		$display("\n=========================================================================");
			...
			if (dut.fsm_inst.state_o == 3'd2) begin
				if (current_pass[0] == 1'b0) begin
					begin : sv2v_autoblock_1
						reg signed [31:0] r;
						for (r = 0; r < 4; r = r + 1)
							if (crossbar_sel[r * 4+:4] !== {1'b0, sv2v_cast_3_signed(r)}) begin
								$display("[FAIL 1x1] Cycle %0d, Pass %0d: Expected crossbar[%0d] = %0d, got %0d", sim_cycle, current_pass, r, r, crossbar_sel[r * 4+:4]);
								err_count = err_count + 1;
							end
					end
					begin : sv2v_autoblock_2
						reg signed [31:0] r;
						for (r = 4; r < 8; r = r + 1)
							if (crossbar_sel[r * 4+:4] !== 4'b1000) begin
					...
					begin : sv2v_autoblock_3
						reg signed [31:0] r;
						for (r = 0; r < 4; r = r + 1)
							if (crossbar_sel[r * 4+:4] !== {1'b0, sv2v_cast_3_signed(4 + r)}) begin
								$display("[FAIL 1x1] Cycle %0d, Pass %0d: Expected crossbar[%0d] = %0d, got %0d", sim_cycle, current_pass, r, 4 + r, crossbar_sel[r * 4+:4]);
								err_count = err_count + 1;
							end
					end
		...
		if (err_count == 0)
			$display(">>> SUCCESS: 1x1 Half-Array Sequencer Verified 100%%! <<<");
		$display("\n=========================================================================");
